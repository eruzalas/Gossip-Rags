...
func _physics_process(delta: float) -> void:
	...
	if (Input.is_action_just_pressed("p1_drop")):
	...
	if (Input.is_action_just_pressed("p1_interact") and costume_in_range and is_on_floor()):
		pickup()
...
func pickup():
	...
	new_costume.remove()
	update_stats()
	#$inventory_ui.update() #Debug UI, not really important
	if (!is_empty):
		toggle_selected()
	update_sprite()
...
func update_sprite():
	var sprite = ["p1_default","p1_banana","p1_motorbike_helmet","p1_pirate","p1_cat","p1_nurse","p1_cult","p1_dino","p1_ghost","p1_ninja","p1_angel","p1_horse"]
	var selected = 0 #
	for i in range (player_inventory.size()):
		if (!player_inventory.equipment[i]):
			pass
